...
fn main() {
	document := xlsx.Document.from_file('spec/04_1MB_file/Free_Test_Data_1MB_XLSX.xlsx')!
	sheet := document.sheets[1]
	println('Sheet 1 has ${sheet.rows.len} rows.')
	...
	if extracted_data != part_data {
		println('Data extraction failed.')
	}
}
