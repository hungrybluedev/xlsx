...
// Employee data from payroll.xlsx spec file (same as spec/06)
const employees = [
...
fn build_payroll_document() !xlsx.Document {
	...
	// Row 2: Column headers (partial - above main headers)
	sheet.set_cell(Location.from_encoding('D2')!, 'Hours Worked')
	sheet.set_cell(Location.from_encoding('E2')!, 'Overtime Hours')
	sheet.set_cell(Location.from_encoding('F2')!, 'Pay')
	sheet.set_cell(Location.from_encoding('G2')!, 'Overtime Bonus')
	sheet.set_cell(Location.from_encoding('H2')!, 'Total')
	...
	// Rows 4-23: Employee data (20 employees) with overtime calculations
	for i, emp in employees {
		...
		// Column E: Overtime Hours = IF(hours > 40, hours - 40, 0)
...
fn test_roundtrip_payroll() ! {
	...
	// Write to temp file
	temp_path := os.join_path(os.temp_dir(), 'test_payroll_overtime_roundtrip.xlsx')
	defer {
		os.rm(temp_path) or {}
	}
	...
	// Verify dimensions (28 rows, 8 columns: A-H)
	assert written_data.raw_data.len == 28, 'should have 28 rows'
	assert written_data.raw_data[0].len == 8, 'should have 8 columns'
	...
	assert written_data.raw_data[1][4] == 'Overtime Hours', 'E2 should be Overtime Hours'
	assert written_data.raw_data[1][5] == 'Pay', 'F2 should be Pay'
	assert written_data.raw_data[1][6] == 'Overtime Bonus', 'G2 should be Overtime Bonus'
	assert written_data.raw_data[1][7] == 'Total', 'H2 should be Total'
	...
	// E4-H4 are formulas - values will be 0 (placeholder) until Excel recalculates

	// Row 24: Empty (sparse row)
	assert written_data.raw_data[23][0] == '', 'A24 should be empty'
	assert written_data.raw_data[23][7] == '', 'H24 should be empty'
